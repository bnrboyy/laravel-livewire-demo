    ...
    <div class="flex flex-wrap gap-2">
        <div class="w-[140px] relative">
            <figure class="group relative h-[145px] shadow-md bg-white rounded-[5px] border border-[#6d71752b]">
                <div class="group-hover:block absolute inset-0 left-0 top-0 border border-solid border-gray-300 rounded-[5px] pointer-events-none"></div>
                <button onclick="removeImg()" class="btn-remove {{ $btnRemove }} w-[25px] h-[25px] flex justify-center items-center rounded-[50%] absolute z-50 right-[2px] top-[3px] bg-red-200 bg-opacity-40 duration-500 hover:bg-red-400"><i class="fa-solid fa-xmark"></i></button>
                @if ($image)
                    <img class="w-full h-full bg-[#e9e9e9] rounded-[4px] object-fill pointer-events-none scale-100 group-hover:scale-90 duration-500" src={{ $image }} id="preview-img" alt="" style="width: 150px;">
                @endif
                <input class="cursor-pointer w-full h-full absolute left-0 opacity-0 top-0" type="file" id="image" wire:change="$emit('imageChoosen')" wire:model="file" accept="image/jpeg, image/png, image/jpg">
            </figure>
        </div>
    </div>
...
<script>
    ...
    function removeImg() {
        file.value = "";
        previewImg.src = 'https://backoffice.juppgas-delivery.shop/images/no-image.png';
        btn_remove.classList.add('hidden');
    }
